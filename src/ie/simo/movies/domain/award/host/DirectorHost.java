...
public class DirectorHost extends Host {

	public DirectorHost(Award award) {
		super();
		setAward(award);
	}
	...
	protected String getNomineeList(List<MovieInfo> nominees, String type) {
		...
		for (MovieInfo info : nominees) {
			String nominee = info.getDirector().getName();
	...
	protected String chooseWinner(List<MovieInfo> nominees) {
		...
		return winner.getDirector().getName() + " for " + winner.getTitle()
				+ "!";
	}
}
